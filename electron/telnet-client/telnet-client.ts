...
export interface SendOptions {
  ...
  stripControls?: boolean;
  timeout?: number;
  waitFor?: string | RegExp | ((message: string) => boolean) | false;
}
...
export class Telnet extends EventEmitter {
  ...
  private opts = Object.assign({}, defaultOptions as Required<ConnectOptions>);
  ...
  private pushNextData(data: any): void {
    if (data instanceof Buffer) data = data.toString(this.opts.encoding);
    else if (data != null) data = data.toString();
  ...
  async nextData(): Promise<string | null> {
    if (this.pendingData.length > 0) return this.pendingData.splice(0, 1)[0];
    else if (this.state === 'end') return null;
  ...
  connect(opts: ConnectOptions): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      const rejectIt = (reason: any): void => {
      ...
      this.opts.initialCtrlC = (opts.initialCtrlC &&
        this.opts.initialCTRLC) as boolean;
      ...
      this.socket.on('data', data => {
        ...
        if (this.state === 'standby' || !this.opts.negotiationMandatory) {
          this.emit(
            'data',
            this.opts.newlineReplace !== undefined
              ? Buffer.from(this.decode(data), this.opts.encoding)
              : data,
          );
          emitted = true;
        }
        ...
        if (
          (data = this.parseData(data, isReady) as Buffer) &&
          connectionPending &&
          (isReady[0] || !this.opts.shellPrompt)
        ) {
          resolveIt();
      ...
      this.socket.on('end', () => {
        ...
        if (connectionPending) {
          if (this.state === 'start') resolveIt();
          else rejectIt(new Error('Socket ends'));
        }
      });
      ...
      this.socket.on('close', () => {
        ...
        if (connectionPending) {
          if (this.state === 'start') resolveIt();
          else rejectIt(new Error('Socket closes'));
        }
      });
      ...
      new Promise((resolve, reject) => {
        ...
        this.socket.write(cmd, () => {
          ...
          const responseHandler = (): void => {
            ...
            if (this.response) {
              const replace = this.opts.newlineReplace;
              resolve(
                this.response.join(replace !== undefined ? replace : '\n'),
              );
  ...
  async write(
    ...
    return asCallback(
      new Promise((resolve, reject) => {
        Object.assign(this.opts, opts || {});
        this.opts.waitFor = opts?.waitFor ?? false;
        stringToRegex(this.opts);
        ...
        if (this.socket.writable) {
          ...
          const sendHandler = (data: Buffer): void => {
            ...
            if (this.opts.waitFor instanceof RegExp) {
              ...
              this.opts.waitFor &&
              {}.toString.call(this.opts.waitFor) === '[object Function]'
            ) {
              if ((this.opts.waitFor as any)(response)) {
                if (sendTimer) clearTimeout(sendTimer);
                this.socket.removeListener('data', sendHandler);
                resolve(response);
              }
  ...
  parseData(chunk: Buffer, isReady?: boolean[]): Buffer | null {
    if (chunk[0] === 255 && chunk[1] !== 255)
      chunk = this.negotiate(chunk) as Buffer;
    ...
    if (this.state === 'getprompt') {
      ...
      if (search(stringData, this.opts.loginPrompt) >= 0) {
        ...
        const shellPrompt =
          this.opts.shellPrompt instanceof RegExp
      ...
      if (this.opts.stripShellPrompt && response.length > 0) {
        ...
        response[idx] =
          search(response[idx], this.opts.shellPrompt) >= 0
            ? response[idx].replace(this.opts.shellPrompt, '')
            : '';
      }

      this.response = response;
      chunk = null as any as Buffer;
  ...
  negotiate(chunk: Buffer): Buffer | null {
  ...
  private decode(chunk: string | Buffer): string {
    ...
    if (this.opts.escapeHandler) {
      chunk?.replace(/\x1B((\[.*?[a-z])|.)/i, seq => {
        ...
        if (response) this.socket.write(response);
    ...
    if (this.opts.newlineReplace !== undefined)
